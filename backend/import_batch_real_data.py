...
async def run_import():
    # 1. Truncate sample data cleanly table by table
    tables = [
    ...
    # 2. Parse all CSV rows into memory
    depts_map = {}      # code -> name
    subjs_map = {}      # code -> dict
    ...
    print(f"Parsed {total_rows} CSV rows into memory objects.")

    # 3. Batch DB Inserts using single Session
    async with async_session_maker() as db:
        # A. Departments
        dept_db_map = {} # code -> dept_id
        for code, name in depts_map.items():
            d = Department(name=name, code=code)
            db.add(d)
            await db.flush()
            dept_db_map[code] = d.id

        # B. Subjects
        subj_db_map = {} # code -> subj_id
        for code, sdict in subjs_map.items():
            s = Subject(
                name=sdict["name"],
                code=code,
                department_id=dept_db_map[sdict["dept_code"]],
            ...
            db.add(s)
            await db.flush()
            subj_db_map[code] = s.id

        # C. Users & Faculty Profiles
        prof_db_map = {} # email -> prof_id
        for email, udict in users_map.items():
            ...
            fp = FacultyProfile(
                user_id=u.id,
                department_id=dept_db_map[udict["dept_code"]],
        ...
        for rnum, rdict in rooms_map.items():
            rm = Classroom(
                ...
                department_id=dept_db_map[rdict["dept_code"]]
            )
            db.add(rm)

        # E. SectionConfigs
        sec_db_map = {} # (dept_code, name) -> sec_id
        for s_key, sdict in secs_map.items():
            ct_id = prof_db_map.get(sdict["class_teacher_email"]) if sdict["class_teacher_email"] else None
            sc = SectionConfig(
                department_id=dept_db_map[sdict["dept_code"]],
                academic_year=sdict["academic_year"],
                name=sdict["name"],
        ...
        with open("import_report.txt", "w", encoding="utf-8") as rf:
