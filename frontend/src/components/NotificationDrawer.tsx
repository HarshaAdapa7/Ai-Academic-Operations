  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black/60 backdrop-blur-md animate-fade-in">
      <div className="absolute inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-dark-900/95 border-l border-dark-800 shadow-2xl flex flex-col">
          
          {/* Drawer Header */}
          <div className="p-6 border-b border-dark-800 bg-dark-950/80 backdrop-blur-xl flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white flex items-center gap-2">
                  Notification Center
                  {unreadCount > 0 && (
                    <span className="px-2 py-0.5 text-xs font-extrabold bg-primary-500 text-white rounded-full">
                      {unreadCount}
                    </span>
                  )}
                </h2>
                <p className="text-xs text-dark-400">Real-time alerts, daily schedule & leave status</p>
              </div>
            </div>
          ...
          {user && ['ADMIN', 'DEAN', 'HOD'].includes(user.role) && (
            <div className="p-4 bg-dark-950/60 border-b border-dark-800/80">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className="p-1 rounded text-dark-400 hover:text-white transition-all"
              ...
              <div className="grid grid-cols-3 gap-2">
                {deptLeaveCounts.length > 0 ? (
                  deptLeaveCounts.map(d => (
                    <div key={d.department_id} className="bg-dark-900 border border-dark-800 rounded-xl p-2 text-center">
                      <div className="text-[10px] font-semibold text-dark-400">{d.department_code}</div>
                      <div className={`text-sm font-extrabold ${d.absent_faculty_count > 0 ? 'text-amber-400' : 'text-emerald-400'}`}>
